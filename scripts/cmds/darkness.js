...
module.exports = {
	config: {
		...
		role: 0,
		description: {
			en: "Apply darkness effect to an image using uid/mention/reply"
		},
		category: "fun",
		guide: {
	...
	onStart: async function ({ message, args, event }) {
		...
		const pathImg = __dirname + `/cache/darkness_${event.senderID}.png`;
		const res = await axios.get(`${APIURL}/api/maker/darkness?url=${encodeURIComponent(url)}&apikey=${APIKEY}&no=40`, { responseType: "arraybuffer" });
